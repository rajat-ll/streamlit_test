...
def edit_ui():
    ...
    selected_tablename = st.sidebar.selectbox(label = "table choice", options = st.session_state.allowed_tables_list, label_visibility="hidden")  # available_tables.name.unique()
    ...
    if selected_tablename != st.session_state.selected_tablename:
        ...
        st.session_state.selected_table_df = st_read_from_snowflake(f"Select * from {st.session_state.selected_tablename}") # Read once to avoid repeated hit on SF server
    ...
    ### Apply filters and display filtered DataFrame
    st.session_state.selected_table_df = st.session_state.selected_table_df if any(value is None for value in st.session_state.filters.values()) else apply_filters(st.session_state.selected_table_df, st.session_state.filters)

    # Editable Pane of the Table
    if st.session_state.right_pane_table_view == True:
        with st.form("data_editor_form"):
            st.caption("Change below to UPDATE DATA in the selected table")
            edited = st.experimental_data_editor(st.session_state.selected_table_df.reset_index(drop=True), use_container_width=True, num_rows="fixed")

            submit_button = st.form_submit_button("Submit Changes")
            st.session_state.submit_button = submit_button
    
    ## EDIT form : INSERT and UPDATE button actions
    if st.session_state.submit_button:
        # print(edited.head(3))  ## check table structure in terminal

        edited_df_pk = edited['ID'].unique()

        if len(edited_df_pk) > 0:
            edited_update_df = edited #[edited['ID'].isin(edited_df_pk)]
            try:
                # print("Creating Update Query")
                ...
                # st.write(f"update query dict {st.session_state.user_query_list}")
                escaped_queries = [q.replace("'", "''") for q in st.session_state.user_query_list]
                user_query_json = json.dumps(escaped_queries)
                # print("DataFrame pushed to Snowflake successfully")
                update_log_query =  f'''INSERT INTO LL_PROD_RAW_ZONE.PUBLIC.STREAMLIT_DATA_EDIT_UI_LOGS (USERNAME , TABLENAME , USER_QUERY , DATETIME) 
    ...
    ## ADDING form : INSERT and UPDATE button actions
    if st.session_state.insert_button:
        ...
        st.write(added_df_pk)
        st.write(st.session_state.master_pk_list)
        ...
        if len(added_insert_pk_list) > 0:
            ...
            try:
                ...
                # st.write(f"insert query dict {st.session_state.user_query_list}")
                escaped_queries = [q.replace("'", "''") for q in st.session_state.user_query_list]
                user_query_json = json.dumps(escaped_queries)
                insert_log_query =  f'''INSERT INTO LL_PROD_RAW_ZONE.PUBLIC.STREAMLIT_DATA_EDIT_UI_LOGS (USERNAME , TABLENAME , USER_QUERY , DATETIME) 
                                        values ('{st.session_state.username}', '{selected_tablename}', '{user_query_json}', '{datetime.now()}');'''
