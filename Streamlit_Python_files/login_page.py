...
def login():
    ...
    # try:
        st.title("Login Page")
        # st.write("<h1 style='text-align: center;'>Login Page</h1>", unsafe_allow_html=True)
        # st.markdown('<div style="margin-bottom:30px;"></div>', unsafe_allow_html=True)
        cols = st.columns([3,10,3])
        with cols[1]:
            st.image("ll.png",  use_column_width=True)
        # st.markdown('<div style="margin-bottom:50px;"></div>', unsafe_allow_html=True)
        cols = st.columns([3,6,6,3]) #,6,3])
        with cols[1]:
        ...
        with cols[1]:
            if st.button("Login"):
                # print(login_creds.loc[login_creds.key == inputkey]['team'].iloc[0])
                if inputkey in login_creds.key.unique():
                    ...
                    if len(st.session_state.allowed_tables_list) == 0:
                        st.session_state.allowed_tables_list.extend(table.strip() for tables in dept_tables.loc[dept_tables['team'].str.contains(dept.values[0]), 'tables'] for table in tables.split(','))
