    ...
    with cols[1]:
        if st.button("Login"):
            if inputkey in login_creds.key.unique():
                ...
                dept = login_creds.loc[login_creds['key'] == inputkey, 'team'].iloc[0]

                if len(st.session_state.allowed_tables_list) == 0:
                    st.session_state.allowed_tables_list.extend(table.strip() for tables in dept_tables.loc[dept_tables['team'] == dept, 'tables'].values for table in tables.split(','))
